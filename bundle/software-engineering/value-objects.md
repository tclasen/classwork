...
A value object is defined by its attributes and behavior rather than a persistent identity. It is commonly immutable, interchangeable with another instance having the same value, and useful for making domain meaning explicit—for example, a measured amount or address. Value objects often live inside an [aggregate](aggregate.md).[^domain-language-reference]
