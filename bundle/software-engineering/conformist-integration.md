...
In [conformist integration](domain-driven-design.md), a downstream [bounded context](bounded-context.md) conforms to an upstream model or protocol instead of imposing its own translation. This can reduce integration cost when the upstream model is adequate, but it leaves the downstream context dependent on upstream choices. It is recorded in a [context map](context-map.md).[^source]
